...
public class CyclopeStatueBehiavor : MonoBehaviour
{
    ...
    public Sprite leftSprite; // utilis� aussi pour droite, mais invers�
    ...
    IEnumerator BehaviorLoop()
    {
        while (true)
        {
            ...
            // D�placer uniquement si la case suivante est libre
            if (!IsBlocked(nextPos))
            {
                direction = newDir; // Mettre � jour la direction
                UpdateSpriteAndCollider();
                yield return MoveOneUnitInDirection();
            }
            else
            {
                // Saut sur place si direction a chang� mais case bloqu�e
                if (axisChange || opposite)
                {
                    // D�j� fait plus haut, donc pas besoin de sauter de nouveau
                }
                else
                {
                    // Case bloqu�e mais pas changement de direction rester sur place
                    yield return JumpInPlace();
                }
            }
        }
    ...
    Vector3 GetNextPosition(CyclopeDirection oldDir, CyclopeDirection newDir)
    {
        ...
        // Si saut sur place, reste � la position actuelle
    ...
    {
        // V�rifie les collisions avec des colliders dans la zone
        Collider2D[] hits = Physics2D.OverlapCircleAll(pos, 0.1f);
        foreach (var hit in hits)
        {
            if (hit == null) continue;

            // Si c'est un collider non trigger et que ce n'est pas un Stats bloqu�
            if (!hit.isTrigger && hit.GetComponent<Stats>() == null)
                return true;

            // Si c'est un trou ("Hole" + chiffre) bloqu�
            if (hit.tag.StartsWith("Hole"))
                return true;
        }

        // Aucun blocage d�tect�
        return false;
    }
