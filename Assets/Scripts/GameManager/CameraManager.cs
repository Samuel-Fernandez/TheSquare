...
public class CameraManager : MonoBehaviour
{
    ...
    public void UpdateFilter()
    {
        switch (filter)
        {
            ...
            case CameraFilter.DUST_STORM:
                ...
                SetFilmGrainEffect(3f, 0.5f); // bruit moyen pour simuler poussi�re
                SetBloomEffect(0.4f, 0.5f); // l�g�re diffusion lumineuse
                SetLensDistortionEffect(-0.1f, 0.5f); // tr�s subtile
                SetDepthOfFieldEffect(0.7f, 8f, 0.5f); // vision moins nette
                break;
            case CameraFilter.OLD:
                SetChromaticAberrationEffect(0.3f, 1f, defaultCamera); // Aberration l�g�re
                ChangeCameraColor(new Color(190f / 255f, 180f / 255f, 150f / 255f), 0.5f, defaultCamera); // Teinte terne
                SetVignetteEffect(0.6f, 0.9f, .2f, defaultCamera); // Vignettage prononc�
                SetFilmGrainEffect(10f, .2f); // Bruit fort pour effet granuleux
                SetBloomEffect(0.5f, .2f); // Lueur tr�s douce
                SetLensDistortionEffect(-1f, .2f); // L�g�re distorsion de lentille
                SetDepthOfFieldEffect(1f, 7f, .2f); // Flou d�arri�re-plan marqu�
                break;
            case CameraFilter.SHYRON:
                SetChromaticAberrationEffect(0.5f, 0.5f, defaultCamera); // Aberration marqu�e
                ...
                SetBloomEffect(0.3f, 0.5f); // Lueur discr�te
                SetLensDistortionEffect(-0.5f, 0.5f); // Distorsion subtile
                SetDepthOfFieldEffect(0.5f, 5f, 0.5f); // L�ger flou d�arri�re-plan
                break;
            ...
            case CameraFilter.BLIZZARD:
                ...
                ChangeCameraColor(new Color(150f / 255f, 180f / 255f, 255f / 255f), 0.5f, defaultCamera); // Bleu glacé / givré
                SetVignetteEffect(0.35f, 0.8f, 0.5f, defaultCamera); // Vignette blanche/froide simulée ou forte assombrissement aux bords
                SetFilmGrainEffect(2f, 0.5f); // Bruit léger pour accompagner les flocons
    ...
    // ShakeCamera Avec cam�ra par d�faut
    ...
    // Remplacez la m�thode ZoomRoutine existante par celle-ci :
