...
public class PlayerManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        InitializeSpecialAttacks(); // Initialisation des attaques sp�ciales runtime
    }
    ...
    IEnumerator DodgeTimeRoutine()
    {
        if (!player.GetComponent<Stats>().isDying)
        {
            ...
            Time.fixedDeltaTime = 0.02f * Time.timeScale; // Pour �viter les bugs physiques

            GetComponent<SoundContainer>().PlaySound("SlowMotion", 1);

            float zoomDuration = 0.25f;
            CameraManager.instance.ZoomCamera(2f, zoomDuration); // Assure-toi que cette m�thode utilise Time.unscaledDeltaTime
            CameraManager.instance.SetChromaticAberrationEffect(2f, zoomDuration); // Assure-toi que cette m�thode utilise Time.unscaledDeltaTime
            CameraManager.instance.SetVignetteEffect(2f, 2f, zoomDuration); // Assure-toi que cette m�thode utilise Time.unscaledDeltaTime

            yield return new WaitForSecondsRealtime(1f); // Pas ralenti

            CameraManager.instance.DezoomCamera(4f, zoomDuration);
            CameraManager.instance.SetChromaticAberrationEffect(0f, zoomDuration); // Assure-toi que cette m�thode utilise Time.unscaledDeltaTime
            CameraManager.instance.SetVignetteEffect(0f, 2f, zoomDuration); // Assure-toi que cette m�thode utilise Time.unscaledDeltaTime


            yield return new WaitForSecondsRealtime(zoomDuration); // attendre la fin du d�zoom si n�cessaire

            Time.timeScale = 1f;
            Time.fixedDeltaTime = 0.02f; // R�tablir le fixedDeltaTime par d�faut
            isDogingTime = false;
        }
        else
        {
    ...
    // Initialisation des attaques sp�ciales
    // Cr�er des copies des ScriptableObjects pour l'ex�cution
    void InitializeSpecialAttacks()
    {
        runtimeSpecialAttacks = new List<SpecialAttack>();

        foreach (SpecialAttack attack in attackDatabase.GetAllSpecialAttacks()) // R�cup�re toutes les attaques sp�ciales depuis la base
        {
            SpecialAttack clonedAttack = ScriptableObjectUtility.Clone(attack); // Cr�e une copie de l'attaque sp�ciale
            runtimeSpecialAttacks.Add(clonedAttack);
        }
    }

    // R�cup�re l'attaque sp�ciale en fonction du nom
    ...
    // BONUSES
    public float regenRate = 0;                        // nb de coeur reg�n�r� toutes les 60 secondes FAIT
    public float doubleSquareCoinsChances = 0;         // % de chance de doubler en tuant FAIT
    public float negativeEffectReducer = 0;            // % de d�g�t / de temps en moins FAIT 
    public float itemsAddChance = 0;                   // Am�lioration statistique des items FAIT
    public float mineralChance = 0;                    // Chance que de meilleurs min�raux apparaissent FAIT
    public float dodgeChance = 0;                      // Chance d'esquiver totalement un d�g�t FAIT
    public float doubleMineralDropChance = 0;          // Chance de doubler un min�ral FAIT
    public float dropChance = 0;                       // Chance de tomber sur un meilleur item lors d'un drop FAIT
    public float pickaxeSpeed = 0;                     // R�duction du temps de la pioche FAIT
    public float bowSpeed = 0;                         // R�duction du temps de l'arc FAIT
    public float shieldKnockback = 0;                  // Knockback ajout� au bouclier FAIT
    ...
    {
        // BONUSES
        float regenRate = 0;                        // nb de coeur reg�n�r� toutes les 60 secondes FAIT
        float doubleSquareCoinsChances = 0;         // % de chance de doubler en tuant FAIT
        float negativeEffectReducer = 0;            // % de d�g�t / de temps en moins FAIT 
        float itemsAddChance = 0;                   // Am�lioration statistique des items FAIT
        float mineralChance = 0;                    // Chance que de meilleurs min�raux apparaissent FAIT
        float dodgeChance = 0;                      // Chance d'esquiver totalement un d�g�t FAIT
        float doubleMineralDropChance = 0;          // Chance de doubler un min�ral FAIT
        float dropChance = 0;                       // Chance de tomber sur un meilleur item lors d'un drop FAIT
        float pickaxeSpeed = 0;                     // R�duction du temps de la pioche FAIT
        float bowSpeed = 0;                         // R�duction du temps de l'arc FAIT
        float shieldKnockback = 0;                  // Knockback ajout� au bouclier FAIT
        float bonusHP = 0;
        float bonusSTR = 0;
        float bonusSPE = 0;
