...
public class GameoverManager : MonoBehaviour
{
    ...
    IEnumerator CloseGameOverRoutine()
    {
        ...
        // 3. Charger (qui va changer de sc�ne avec transition)
        ...
        // 4. Attendre que la sc�ne soit compl�tement charg�e
        while (!ScenesManager.instance.isSceneLoaded)
        {
            yield return null;
        }

        // 5. R�activer le joueur et sauvegarder
    ...
    public void RestartSoulMoneyReductionRoutine()
    {
        if (isSoul)
        {
            if (soulMoneyReductionRoutine != null)
            {
                StopCoroutine(soulMoneyReductionRoutine); // Arr�te la coroutine en cours
            }
            soulMoneyReductionRoutine = StartCoroutine(SoulMoneyReductionRoutine()); // D�marre une nouvelle coroutine
        }
    }
}
