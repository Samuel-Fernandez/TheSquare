...
public class InventoryManager : MonoBehaviour
{
    ...
    {
        OpenStats();
        defaultFixedDeltaTime = Time.fixedDeltaTime; // Sauvegarde la valeur par d�faut
        ...
        if (canOpenInventory)
        {
            if (PlayerManager.instance.playerInputActions.Menu.Pause.triggered || var)
            {
                ...
                if(!PlayerLevels.instance.UIPlayerLevels.activeSelf && (!isSealUIAffixed || inventory.activeSelf))
                {
                    ...
                    {
                        Time.timeScale = 1f;
                        Time.fixedDeltaTime = defaultFixedDeltaTime; // R�initialise fixedDeltaTime
