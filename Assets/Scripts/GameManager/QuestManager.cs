...
public class QuestManager : MonoBehaviour
{
    ...
    // Cens� contenir toutes les qu�tes
    public List<Quests> questsDB;

    // Qu�tes termin�es ou en attente (identifiant)
    ...
    // Text � mettre � jour avec les infos de la qu�te
    ...
    // Contient tous les selecteurs de qu�te
    ...
    {
        // Cr�er un QuestContainer correspondant
        ...
        // Ajouter la qu�te au journal des qu�tes termin�es
        ...
        // Retirer la qu�te des qu�tes en attente
        ...
        //Retire les ressources demand�es
        ...
        foreach (var waitingQuest in waitingQuests)
        {
            foreach (var quest in questsDB)
            {
                if (quest.id == waitingQuest.id)
                {
                    ...
                    // R�cup�ration des nbMonsterKilled...
    ...
    public void AcceptQuest()
    {
        ...
        // Initialiser les listes avec des valeurs par d�faut vides
        ...
        // Ajouter la qu�te en attente avec les donn�es n�cessaires
    ...
    {
        // R�initialiser la qu�te actuelle
        ...
        // Supprimer les slots de r�compenses
        ...
        switch (quest.reward)
        {
            ...
            case QuestReward.EQUIPEMENT:
                ...
                foreach (var item in actualQuest.rewardEquipement)
                {
                    ...
                    // Cr�er l'interface avec la copie
                    GameObject rewardSlot = Instantiate(rewardSlotPrefab, rewardContainer.transform);
    ...
    public bool ObjectivesDone(Quests quest)
    {
        ...
        // R�cup�rer la qu�te en attente correspondante
        QuestContainer waitingQuest = waitingQuests.FirstOrDefault(q => q.id == quest.id);
        if (waitingQuest == null)
        {
            return false;  // Si la qu�te n'est pas en attente
        }

        // Cloner la qu�te pour comparaison
        Quests questCopy = ScriptableObjectUtility.Clone(quest);
        ...
        switch (quest.completionCondition)
        {
            case QuestCompletionCondition.KILL_MONSTER:
                ...
                {
                    // Chercher le nombre de monstres tu�s avec l'id sp�cifique
    ...
    // V�rifie si une qu�te est dans la liste des qu�tes en attente
    public bool IsInWaiting(Quests quest)
    {
        return quest != null && waitingQuests.Any(q => q.id == quest.id);
    }

    // V�rifie si une qu�te est dans la liste des qu�tes termin�es
