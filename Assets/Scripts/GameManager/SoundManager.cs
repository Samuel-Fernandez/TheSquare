...
public class SoundManager : MonoBehaviour
{
    ...
    // M�thode pour changer le volume de la musique
    ...
    // M�thode pour changer le volume du son
    ...
    public void PlaySound(AudioClip sound, float pitch, Vector3? sourcePosition = null)
    {
        ...
        AudioSource audioSource = gameObject.AddComponent<AudioSource>(); // Cr�e une nouvelle instance d'AudioSource
        audioSource.pitch = pitch;
        ...
        audioSource.PlayOneShot(sound);
        Destroy(audioSource, sound.length); // D�truit l'AudioSource apr�s la fin du son
    }

    // Joue un AudioClip al�atoire � partir d'une liste avec position
    public void PlaySound(List<AudioClip> sounds, float pitch, Vector3? sourcePosition = null)
    {
        if (sounds == null || sounds.Count == 0) return; // V�rifie que la liste n'est pas vide

        int index = Random.Range(0, sounds.Count); // S�lectionne un index al�atoire
        PlaySound(sounds[index], pitch, sourcePosition); // Utilise la premi�re m�thode PlaySound
    }

    private float CalculateVolumeByDistance(Vector3 sourcePosition)
    {
        float maxVolume = PlayerPrefs.GetFloat(SOUND_VOLUME_KEY);
        float maxDistance = 15f; // Distance maximale pour entendre le son � son volume maximal
    ...
    public void PlayMusic(AudioClip clip)
    {
        ...
        // Si d�j� en train de jouer ce clip, on ne fait rien
        ...
        // Si on n'a pas trouv� d'ID, utiliser le nom du clip
        ...
        // Arr�ter la transition en cours si n�cessaire
        StopMusicTransition();

        // D�marrer une nouvelle transition
        musicTransitionCoroutine = StartCoroutine(TransitionToNewMusic(clip));
        isTransitioning = true;

        // Mettre � jour les r�f�rences actuelles
    ...
    public void PlayMusic(string id)
    {
        ...
        // V�rifier si la musique actuelle correspond � l'ID
    ...
    {
        // Charger le clip en m�moire avant de le jouer
