...
public class WagonBehavior : MonoBehaviour
{
    ...
    // Direction actuelle du wagon (normalis�e)
    private Vector2 currentDirection = Vector2.zero;

    // R�f�rence au joueur mont� sur le wagon
    private GameObject mountedPlayer = null;

    // Dernier rail trait� pour �viter les doubles d�tections
    ...
    private IEnumerator HandleAxisChange(TrackBehavior track)
    {
        ...
        // Dur�e totale identique � avant (0.175 + 0.175 = 0.35)
        float duration = 0.35f;
        Vector2 startPos = transform.position;
        Vector2 endPos = startPos + newDirection; // d�placement d'1 unit� dans la nouvelle direction

        // Mettre � jour la direction imm�diatement pour coh�rence (dismount, jump, etc.)
    ...
    private IEnumerator ResetLastProcessedTrack(TrackBehavior track)
    {
        ...
        if (distance > 1f)
        {
            ...
            Debug.Log("[ResetLastProcessedTrack] Rail r�initialis�");
        }
    }
    ...
    private IEnumerator MountPlayerRoutine()
    {
        ...
        foreach (var hit in hits)
        {
            ...
            if (track != null && track.type == TrackType.END)
            {
                ...
                Debug.Log($"[MountPlayerRoutine] D�marrage - Direction: {currentDirection}");
                break;
            }
        }
    ...
    private IEnumerator DismountPlayer()
    {
        ...
        Debug.Log("[DismountPlayer] D�but de la descente");
        ...
        Debug.Log("[DismountPlayer] Descente termin�e - Reset complet");
    }
}