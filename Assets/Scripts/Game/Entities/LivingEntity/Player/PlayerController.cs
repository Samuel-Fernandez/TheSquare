...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Obtenir la r�f�rence au SpecialAttackController
    ...
    IEnumerator PerformDodge()
    {
        ...
        List<GameObject> projectilesBeingDodged = new List<GameObject>(); // Tracker les projectiles s�par�ment

        // Trouver tous les objets � d�sactiver temporairement
        foreach (GameObject obj in GameObject.FindObjectsOfType<GameObject>())
        {
            ...
            if (obj.CompareTag("Projectile"))
            {
                foreach (var playerCol in playerColliders)
                    Physics2D.IgnoreCollision(playerCol, otherCol, true);
                ignoredColliders.Add(otherCol);
                projectilesBeingDodged.Add(obj); // Ajouter � la liste des projectiles

                // V�rification imm�diate pour les projectiles d�j� proches
        ...
        while (elapsedTime < dodgeDuration)
        {
            ...
            // V�rifier les projectiles (utiliser la distance car les collisions sont d�sactiv�es)
    ...
    // Version am�lior�e de la d�tection par distance
    private bool IsPlayerOverlappingEntityByDistance(GameObject entity)
    {
        ...
        // Essayer d'obtenir la taille r�elle du collider pour une d�tection plus pr�cise
        Collider2D entityCollider = entity.GetComponent<Collider2D>();
        if (entityCollider != null)
        {
            // Utiliser les bounds du collider pour une d�tection plus pr�cise
    ...
    // Alternative : D�tection par bounds (plus pr�cise que la distance)
    private bool IsPlayerOverlappingEntityByBounds(GameObject entity)
    {
        ...
        // V�rifier si les bounds se chevauchent
    ...
    // M�thode avec raycast (pour d�tecter m�me � travers les collisions d�sactiv�es)
    ...
    // Coroutine pour mettre � jour la scrollbar
    ...
    private bool holdAttackRegistered = false; // Nouvelle variable pour s'assurer que RegisterInput(HoldAttack) n'est ex�cut� qu'une fois
    private bool releaseAttackRegistered = false; // Nouvelle variable pour s'assurer que RegisterInput(ReleaseAttack) n'est ex�cut� qu'une fois
    ...
    void Attack()
    {
        // Si le bouton d'attaque est enfonc� et que le joueur peut attaquer
        if (playerInputActions.Gameplay.Attack.triggered && CanAttack)
        {
            ...
            // Sinon, ex�cuter l'attaque normale uniquement si le joueur n'est pas d�j� en train d'attaquer
            else if (!isAttacking)
            {
                ...
                holdAttackTime = 0f;
                holdAttackRegistered = false; // R�initialiser le flag de HoldAttack
                releaseAttackRegistered = false; // R�initialiser le flag de ReleaseAttack
                actualSpeed = 0;
        ...
        if (PlayerManager.instance.GetSpecialAttack("BigSlash").isAvailable && playerInputActions.Gameplay.Attack.IsPressed() && !isAttacking)
        {
            if (isHoldingAttack && attackAnimationCompleted)
            {
                ...
                actualSpeed = 0; // Maintenir la vitesse � 0 pendant l'attaque maintenue
                ...
                // Jouer l'animation appropri�e en fonction de la direction
                UpdateHoldAttackAnimation();
            }

        }
        // Dans la m�thode Attack(), partie 'else' lorsque le bouton est rel�ch�
        else
        {
            // Si le bouton est rel�ch�, arr�ter l'attaque maintenue
            if (isHoldingAttack)
            {
                // Si l'attaque a �t� maintenue pendant au moins 1 seconde et que ReleaseAttack n'a pas encore �t� enregistr�
                if (holdAttackTime >= 1.0f && !releaseAttackRegistered && attackAnimationCompleted)
                {
                    ...
                    if (specialAttack != null)
                    {
                        ...
                        // Ne pas restaurer la vitesse ici, car l'attaque sp�ciale va s'en charger
                    }
                    else
                    {
                        // Si aucune attaque sp�ciale n'est d�clench�e, restaurer la vitesse
                        actualSpeed = stats.speed;
                    }
                }
                else
                {
                    // Si l'attaque a �t� maintenue moins d'une seconde ou que l'animation initiale n'est pas termin�e
                ...
                releaseAttackRegistered = false; // R�initialiser ce drapeau ici
            }
        }
    ...
    // Nouvelle m�thode pour mettre � jour l'animation d'attaque maintenue
    void UpdateHoldAttackAnimation()
    {
        ObjectAnimation animator = GetComponent<ObjectAnimation>();

        // D�terminer quelle animation jouer en fonction de la direction
    ...
    // M�thode pour ex�cuter une attaque sp�ciale quand un combo est compl�t�
    private void ExecuteSpecialAttack(string attackName)
    {
        // Faire ici les diff�rentes attaques sp�ciales 
        switch (attackName)
        {
            case "Tornado":
        ...
        while (elapsedTime < 0.15f)
        {
            ...
            foreach (Collider2D hitEntity in hitEntities)
            {
                if (!hitEntity.isTrigger)
                {
                    ...
                    // Si une entit� bloque les attaques du joueur
            ...
            // Si une entit� a bloqu� l�attaque on arr�te tout imm�diatement
            if (attackBlocked)
            {
                isAttacking = false;
    ...
    public Vector2 GetAttackDirection()
    {
        ...
        // Si le joueur ne bouge pas, retourner la derni�re direction d'attaque
        if (horizontalInput == 0 && verticalInput == 0)
        {
            return lastAttackDirection;
        }

        // Calculer la direction d'attaque en fonction des entr�es
        Vector2 attackDirection = Vector2.right * horizontalInput + Vector2.up * verticalInput;

        // Normaliser la direction d'attaque
        attackDirection.Normalize();

        // Mettre � jour la derni�re direction d'attaque
    ...
    // NOUVEAU : M�thode pour mettre � jour la position de backup
    private void UpdateSafeBackupPosition()
    {
        // Mettre � jour toutes les 0.3 secondes quand le joueur n'est pas en train de tomber
        if (!fallIntoHole && !isTeleporting && Time.time - lastSafeBackupTime > 0.3f)
        {
            ...
            // Sinon, utiliser la position actuelle si elle est s�re (pas dans un trou)
    ...
    // NOUVEAU : V�rifier si une position est dans un trou
    ...
    // MODIFI� : Appeler UpdateSafeBackupPosition dans Update ou LateUpdate
    ...
    private bool ShouldFallInHole(Tilemap tilemap, out Vector3 holeWorldPosition)
    {
        ...
        // Chercher le trou dans les tuiles adjacentes pour g�rer tous les cas d'approche
        Vector3Int playerTilePosition = tilemap.WorldToCell(playerCenter);
        Vector3Int holeTilePosition = Vector3Int.zero;
        bool holeFound = false;

        // V�rifier la tuile actuelle et les tuiles adjacentes (priorit� aux directions cardinales)
        Vector3Int[] tilesToCheck = {
        playerTilePosition,                    // Centre
        playerTilePosition + Vector3Int.down,  // Bas (priorit� pour approche par le haut)
        ...
        // Obtenir la taille d'une tuile pour d�terminer si le joueur est suffisamment centr�
        Vector3 cellSize = tilemap.cellSize;
        float maxDistanceFromCenter = Mathf.Min(cellSize.x, cellSize.y) * 0.4f; // 40% de la taille de tuile

        // Le joueur doit �tre suffisamment proche du centre du trou pour tomber
        if (distanceToHoleCenter <= maxDistanceFromCenter)
        {
            return true;
        }

        // Alternative : v�rifier si suffisamment du collider du joueur est au-dessus du trou
    ...
    private bool CheckColliderOverlapWithHole(Tilemap tilemap, Vector3Int holeTilePosition, Vector2 playerCenter, float radius)
    {
        ...
        // Test des points sur le p�rim�tre du collider
        for (int i = 0; i < numPoints; i++)
        {
            float angle = (i * 2 * Mathf.PI) / numPoints;
            Vector2 testPoint = playerCenter + new Vector2(
                Mathf.Cos(angle) * radius * 0.8f, // 80% du rayon pour �viter les bords
    ...
    void FallIntoHole(Vector3 holeCenter)
    {
        if (!fallIntoHole && !isTeleporting)
        {
            ...
            // CRITIQUE : Capturer IMM�DIATEMENT toutes les destinations possibles
            // avec ordre de priorit� clair
            Vector3 destinationPosition = Vector3.zero;
            bool hasValidDestination = false;

            // 1. Priorit� : safeTeleportation (si valide et loin du trou)
            if (safeTeleportation != null)
            {
            ...
            // 3. Fallback : lastPosition (ancien syst�me)
            ...
            // 4. Dernier recours : chercher une position s�re autour du trou
            if (!hasValidDestination)
            {
                Debug.LogError("FallIntoHole: Aucune destination valide! Recherche d'une position s�re...");
            ...
            if (!hasValidDestination)
            {
                Debug.LogError("FallIntoHole: IMPOSSIBLE de trouver une destination s�re! Annulation de la chute.");
                fallIntoHole = false;
                isTeleporting = false;
                return;
            }
    ...
    // NOUVEAU : Trouver une position s�re autour du trou
    private Vector3 FindSafePositionAroundHole(Vector3 holeCenter)
    {
        ...
        foreach (var dir in directions)
        {
            ...
            {
                Debug.Log($"Position s�re trouv�e: {testPos}");
                return testPos;
            }
        }

        Debug.LogError("Impossible de trouver une position s�re!");
        return Vector3.zero;
    }
    ...
    IEnumerator FallIntoHoleRoutine(Vector3 holeCenter, Vector3 destinationPosition)
    {
        ...
        // T�l�portation vers la destination pr�-calcul�e
        Debug.Log($"T�l�portation vers: {destinationPosition}");
        ...
        // Mettre � jour le backup avec la nouvelle position s�re
        safeBackupPosition = destinationPosition;
        lastSafeBackupTime = Time.time;

        // CRITIQUE : Petit d�lai avant de permettre une nouvelle chute
        // Cela �vite que le joueur retombe imm�diatement s'il r�appara�t pr�s d'un trou
