...
public class BigBlobBehiavor : MonoBehaviour
{
    ...
    public float smallJumpHeight = 0.7f;   // Hauteur des petits sauts de d�placement
    ...
    bool isMoving = false; // Pour �viter que d�placement et attaque se chevauchent
    ...
    private void Start()
    {
        ...
        else
        {
            ...
            StartCoroutine(BehaviorRoutine()); // Routine principale qui g�re attaque et d�placement
    ...
    // Routine principale qui gre le comportement du blob
    IEnumerator BehaviorRoutine()
    {
        while (true)
        {
            ...
            {
                // Dcision entre attaque et dplacement
                float attackChance = 0.3f; // 30% de chance d'attaquer, 70% de se dplacer

                if (Random.value <= attackChance)
                {
                    // Attaque
                    yield return StartCoroutine(WaitWhileCantMove(Random.Range(0, 1f)));
                    yield return StartCoroutine(AttackJumpRoutine());
                }
                else
                {
                    // Petit saut de dplacement vers le joueur
                    yield return StartCoroutine(WaitWhileCantMove(Random.Range(0, 1.5f)));
                    yield return StartCoroutine(MovementJumpRoutine());
                }
            }
            else
            {
                yield return StartCoroutine(WaitWhileCantMove(0.5f));
            }
        }
    }

    // Vrifie si une position est libre d'obstacles (seul le joueur est accept)
    ...
    Vector3 FindValidPositionAroundPlayer(int maxAttempts = 10)
    {
        ...
        // Si aucune position valide n'est trouv�e, retourner la position du joueur
        return player.position;
    }

    // Petit saut de d�placement vers le joueur
    IEnumerator MovementJumpRoutine()
    {
        ...
        // Distance de d�placement plus courte pour le mouvement
        ...
        if (!IsPositionClear(targetPosition))
        {
            ...
            if (!IsPositionClear(targetPosition))
            {
                // Si toujours pas libre, annuler le d�placement
                isMoving = false;
                yield break;
            }
        }
    ...
    IEnumerator AttackJumpRoutine()
    {
        ...
        yield return StartCoroutine(WaitWhileCantMove(0.8f, true));

        // �tape 2 : Trouver une position valide autour du joueur
        Vector3 targetPosition = FindValidPositionAroundPlayer();

        // Calculer la distance actuelle pour d�terminer la hauteur du saut
    ...
    IEnumerator PerformJump(Vector3 targetPosition, float jumpHeight, float jumpDuration, bool isAttack)
    {
        ...
        while (elapsed < jumpDuration)
        {
            ...
            // D�placer l'ombre en conservant l'offset d'origine mais sans le d�placement vertical du saut
        ...
        // Effets sp�ciaux seulement pour les attaques
        if (isAttack)
        {
    ...
    IEnumerator RandomSoundRoutine()
    {
        while (true)
        {
            ...
            yield return StartCoroutine(WaitWhileCantMove(Random.Range(1f, 5f)));
        }
    }
    ...
    private IEnumerator WaitWhileCantMove(float time, bool unscaled = false)
    {
        float timer = 0f;
        while (timer < time)
        {
