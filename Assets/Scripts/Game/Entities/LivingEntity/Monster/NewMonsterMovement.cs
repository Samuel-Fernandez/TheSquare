...
public class NewMonsterMovement : MonoBehaviour
{
    ...
    private void ApplyMovement(float deltaTime)
    {
        ...
        // V�rifie si on suit un target pr�cis
        if (target != null)
        {
            float distance = Vector3.Distance(transform.position, target.position);

            // Ne rien faire si on est tr�s proche de la cible
            if (distance < 0.05f)
            {
    ...
    private IEnumerator RandomMovementRoutine()
    {
        ...
        {
            // G�n�ration d'une direction al�atoire
