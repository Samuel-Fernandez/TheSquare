...
public class Seal : ScriptableObject
{
    ...
    // G�n�re le sceau � partir de 4 items sp�ciaux
    public void GenerateSeal(SpecialItems[] specialItems)
    {
        ...
        // Pour chaque item sp�cial
        foreach (var item in specialItems)
        {
            if (item == null || item.essenceComposition == null) continue;

            // R�cup�rer les essences normalis�es de l'item
        ...
        // Optionnel : Trier par pourcentage d�croissant pour plus de clart�
        essenceComposition.Sort((a, b) => b.percentage.CompareTo(a.percentage));

        // G�n�rer les stats du sceau bas�es sur cette composition
        GenerateSealStats();
    }

    // G�n�re les statistiques du sceau bas�es sur la composition d'essences
    ...
    private void DetermineArchetype()
    {
        ...
        // Calculer les scores de chaque arch�type (somme pond�r�e des votes)
        foreach (var essenceComp in essenceComposition)
        {
            if (essenceComp.essence == null) continue;
        ...
        // D�terminer quels arch�types sont actifs
        ...
        // D�finir les effets pour les arch�types actifs
    ...
    public Spirit GetDominantSpiritForArchetype(string archetype)
    {
        ...
        foreach (var comp in essenceComposition)
        {
            ...
            // V�rifier si cette essence a des votes pour cet arch�type
            int votes = 0;
            switch (archetype)
            {
